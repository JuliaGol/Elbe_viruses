...
bac_abund <- read.csv("data/BacterialAbundance.csv", header=TRUE, sep=",", row.names=1) 
#unify values
#dates
...
```{r load motus}
#load motus metadata
motus_final_taxa_metadata <- readRDS(file="data/motus_final_taxa_metadata.RDS")
motus_stats <- readRDS(file="data/motu_stats.RDS")
```
...
#inf values to 0 
motus_expression[motus_expression==Inf] <- 0
motus_expression[motus_expression==-Inf] <- 0
motus_expression  <- motus_expression %>% mutate_all(~ifelse(is.nan(.), 0, .))

#recover mOTUs columns
motus_expression <-as.data.frame(cbind(motus_df_METAT$mOTU, motus_expression))
#unify name
colnames(motus_expression)[1]<-"mOTU"
#deduct min to remove negative values
motus_expression[, -c(1)] <- motus_expression[, -c(1)] - min(motus_expression[, -c(1)])
#METAG FL
...
geneabund_drep_marker_taxa <- readRDS("data/vibrant_annot_drep_marker_abund.RDS")
...
saveRDS(viral_diversity_FL, file = "data/viral_diversity_FL.RDS")
saveRDS(viral_diversity_LF, file = "data/viral_diversity_LF.RDS")
saveRDS(viral_diversity_HF, file = "data/viral_diversity_HF.RDS")
...
#inf values to 0 
viral_expression[viral_expression==Inf] <- 0
viral_expression[viral_expression==-Inf] <- 0
viral_expression  <- viral_expression %>% mutate_all(~ifelse(is.nan(.), 0, .))
#deduct minimum to remove non negative values
viral_expression <- viral_expression - min(viral_expression)
...
saveRDS(viral_expression_FL, file = "data/viral_expression_FL.RDS")
saveRDS(viral_expression_LF, file = "data/viral_expression_LF.RDS")
saveRDS(viral_expression_HF, file = "data/viral_expression_HF.RDS")
```
...
saveRDS(metadata_METAG_FL, file="data/metadata_METAG_FL.RDS")
saveRDS(metadata_METAG_LF, file="data/metadata_METAG_LF.RDS")
saveRDS(metadata_METAG_HF, file="data/metadata_METAG_HF.RDS")
saveRDS(metadata_METAT_FL, file="data/metadata_METAT_FL.RDS")
saveRDS(metadata_METAT_LF, file="data/metadata_METAT_LF.RDS")
saveRDS(metadata_METAT_HF, file="data/metadata_METAT_HF.RDS")
...
host_motus_expression_FL.dist <- vegdist(t(host_motus_expression_FL[,-1]), method = "bray", na.rm = T)
...
motus_expression_FL.dist <- vegdist(t(motus_expression_FL[, -c(1)]), method = "bray", na.rm = T)
...
viral_expression_FL.dist <- vegdist(t(viral_expression_FL), method = "bray", na.rm = T)
#param distances
...
tiff("mantel_test_METAG_FL_plot_avrdist_bray_bac_counts.tiff",width=870,height=300,units="px")
...
tiff("mantel_test_METAT_FL_plot_log_euclidean_bac_counts.tiff",width=770,height=300)
...
host_motus_expression_LF.dist <- vegdist(t(host_motus_expression_LF[,-1]), method = "bray", na.rm = T)
#we can take only those into account for parameter distances calculations for each dataset (METAG and METAT) separately
...
motus_expression_LF.dist <- vegdist(t(motus_expression_LF[, -c(1)]), method = "bray", na.rm = T)
#keep the same samples for viral data
...
viral_expression_LF.dist <- vegdist(t(viral_expression_LF), method = "bray", na.rm = T)
...
tiff("mantel_test_METAG_LF_plot_avrdist_bray_bac_counts.tiff",width=870,height=300,units="px")
...
tiff("mantel_test_METAT_LF_plot_log_euclidean_bac_counts.tiff",width=770,height=300)
...
motus_expression_HF.dist <- vegdist(t(motus_expression_HF[, -c(1)]), method = "bray", na.rm = T)
#keep the same samples for viral data
...
viral_expression_HF.dist <- vegdist(t(viral_expression_HF), method = "bray", na.rm = T)
#param distances
#store in a list of distances 
...
tiff("mantel_test_METAG_HF_plot_avrdist_bray_bac_counts.tiff",width=870,height=300,units="px")
...
tiff("mantel_test_METAT_HF_plot_log_euclidean_bac_counts.tiff",width=750,height=300)
...
dev.off()
tiff("mantel_test/mantel_plots/grid_METAT.tiff", unit="px", width = 750, height = 650, res=300)
...
matrix_res_stat_LF_mantel_plot$Fraction <- rep(c("Suspended particles"), each = nrow(matrix_res_stat_LF_mantel_plot))
matrix_res_stat_HF_mantel_plot$Fraction <- rep(c("Sinking particles"), each = nrow(matrix_res_stat_HF_mantel_plot))
...
matrix_res_stat_all_mantel_plot$Fraction <- factor(matrix_res_stat_all_mantel_plot$Fraction, levels= c("Free-living", "Suspended particles", "Sinking particles"))
...
# filter(Var1 != "MOTUs") %>% 
 ...
 mutate(Var1=str_replace(Var1, "_", " "))  %>%
  mutate(Var2=str_replace(Var2, "_", " "))  %>%
  mutate(Var1=str_replace(Var1, "Stromkilometer", "Elbe km"))  %>%
  mutate(Var1=str_replace(Var1, "bacteria counts", "Bacteria Counts"))  %>%
  mutate(Var1=str_replace(Var1, "TotalDissolvedPhosphate", "TDP"))  %>%
  mutate(Var1=str_replace(Var1, "ParticleArea", "Particle Area"))  %>%
...
tiff("mantel_plots/facet_grid_METAG.tiff", unit="cm", height=21.0, width=29.7, res=300)
mantel_test_METAG_plot 
dev.off()
pdf("mantel_plots/facet_grid_METAG.pdf", height=21.0/2, width=29.7/2)
mantel_test_METAG_plot 
dev.off()
# plot for METAT - use normal p -values as significant 
 ...
 # guides(colour = guide_colorbar(title = "Mantel's r", frame.colour = "black", barwidth = 1.5)) +    #title.theme = element_text(angle=90, vjust=1))) +
  ...
  theme(rect = element_blank(),
        axis.text.x = element_text(angle = 90, hjust=0.95, vjust=0.2), 
         base_size = 20, 
        legend.position = "right") +  geom_text(vjust=0.76,hjust=0.49, aes(label = paste0(c("","*")[(abs(p_value.adj) <= .05)+1], c("","*")[(abs(p_value.adj) <= .001)+1], c("","*")[(abs(p_value.adj) <= .001)+1])), color="black") + facet_grid(row="Fraction", scale="free")
tiff("mantel_plots/facet_grid_METAT.tiff", unit="cm", height=21.0, width=29.7, res=300)
mantel_test_METAT_plot 
dev.off()
pdf("mantel_plots/facet_grid_METAT.pdf", height=21.0/2, width=29.7/2)
mantel_test_METAT_plot 
dev.off()
