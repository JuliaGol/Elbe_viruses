...
```{r}
bac_rel_abund_plot <- motus_final_taxa_metadata  %>% filter(counts>10) %>% group_by(class, station_km, data_type, date,  Salinity_level) %>% summarise(counts=sum(counts)) %>% ggplot() + geom_bar(position ="fill", stat = "identity", aes(x= station_km, y=counts, fill = class), show.legend = T)  + scale_y_continuous(labels=scales::percent_format()) + ylab("Relative abundace") + theme(axis.title.y = element_text(angle=90, size=12)) + xlab("Salinity niche") + facet_grid(~date)
bac_rel_abund_plot
```