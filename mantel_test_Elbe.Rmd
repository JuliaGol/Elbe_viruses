...
---
#1. Introduction
In this script we will calculate distances between samples using environmental parameters, MOTUs, hosts MOTUs and viruses abundances and expression of individual marker genes (activity) data to finally get mantel test results which will be visualised. 
...
motus_expression <-exp(motus_expression)
...
#[, colnames(motus_expression) %in% colnames(motus_df_METAT_FL)]  
#expression LF
...
# host_MAGS_motus <- host_prediction_genome %>% left_join(motus_MAGS_METAG, by =c("Host.genome" = "user_genome"))
#host motus
...
#remove last a few columns and take unique rows only  
geneabund_drep_marker_taxa <- distinct(geneabund_drep_marker_taxa[,c(ncol(geneabund_drep_marker_taxa)-9:ncol(geneabund_drep_marker_taxa))])
...
# LF_accnum <- metadata$AccessionNumber_TBDSven[which(metadata$Sample_type=="Light_fraction")] 
# HF_accnum <- metadata$AccessionNumber_TBDSven[which(metadata$Sample_type=="Heavy_fraction")] 
#go back to counts - log values cannot be used for distance matrix for bray cutis method 
viral_activity <-exp(viral_activity)
#FL
...
```
## 2.6 ? all viral expression and metabolic potential??
...
for (param_name in vec_param) { #columns with parameters
  metadata_METAG_FL[,param_name] <- as.numeric(metadata_METAG_FL[,param_name]) #to double
    ...
    if (sum(!is.na(metadata_METAG_FL[,param_name]))>0){ #if most is NA do not calculate distance matrixes #we can later tray with most NA (sum(!is.na(metadata_METAG[,i]))/length(metadata_METAG[,i]))>0.5 
  j=j+1
  dist_METAG_FL <- dist(metadata_METAG_FL[,param_name], method = "euclidian")
  ...
  z=z+1
  dist_METAT_FL <- dist(metadata_METAT_FL[,param_name], method = "euclidian")
...
motus_df_METAG_FL.dist <- vegdist(t(motus_df_METAG_FL[,-1]), "bray", na.rm = T)
motus_expression_FL.dist <- vegdist(t(motus_expression_FL[,-1]), "bray", na.rm = T)

host_motus_METAG_FL.dist <- vegdist(t(host_motus_METAG_FL[,-1]), "bray", na.rm = T)
host_motus_expression_FL.dist <- vegdist(t(host_motus_expression_FL[,-1]), na.rm = T, "bray")

viral_diversity_FL.dist <- vegdist(t(viral_diversity_FL), "bray", na.rm = T)
viral_activity_FL.dist <- vegdist(t(viral_activity_FL), "bray", na.rm = T)
...
if (names(list.METAT_FL.dists)[i] == "viral_activity" & colnames(matrix_res_sig_METAT_FL)[j] == "viral_activity" ){
      next
    }  
...
# matrix_res_stat_METAT_FL <- matrix_res_stat_METAT_FL[,c((length(matrix_res_stat_METAT_FL[1,])-2):length(matrix_res_stat_METAT_FL[1,]))]
# matrix_res_sig_METAT_FL <- matrix_res_sig_METAT_FL[,c((length(matrix_res_stat_METAT_FL[1,])-2):length(matrix_res_stat_METAT_FL[1,]))]
# matrix_res_stat_METAT_FL["viral_activity", "MOTUs"] <- matrix_res_stat_METAT_FL["MOTUs","viral_activity"] 
# matrix_res_stat_METAT_FL <- matrix_res_stat_METAT_FL[,c((length(matrix_res_stat_METAT_FL[1,])-2):length(matrix_res_stat_METAT_FL[1,]))]
# matrix_res_sig_METAT_FL["viral_activity", "MOTUs"] <- matrix_res_sig_METAT_FL["MOTUs","viral_activity"] 
# matrix_res_sig_METAT_FL <- matrix_res_sig_METAT_FL[,c((length(matrix_res_stat_METAT_FL[1,])-2):length(matrix_res_stat_METAT_FL[1,]))]
library(reshape2)
...
#p_value.adj_FL <- matrix_res_sig_METAT_FL %>% filter(Var2 =="MOTUs_FL") p.adjust(matrix_res_sig_METAT_FL$p_value, method="hochberg")
...
# #remove false values
# matrix_res_sig_METAG_FL <- matrix_res_sig_METAG_FL[which(!(matrix_res_sig_METAG_FL$Var1%in% c("Total_DIN_µM", "Ammonium_µM", "Phosphate_µM"))),] 
# matrix_res_stat_METAG_FL <- matrix_res_stat_METAG_FL[which(!(matrix_res_stat_METAG_FL$Var1%in% c("Total_DIN_µM", "Ammonium_µM", "Phosphate_µM"))),] 
...
# levels(matrix_res_stat_FL_mantel_plot$Var1)[match("viral_activity",levels(matrix_res_stat_FL_mantel_plot$Var1))] <- "viral_activity" 
# levels(matrix_res_stat_FL_mantel_plot$Var2)[match("viral_activity",levels(matrix_res_stat_FL_mantel_plot$Var2))] <- "viral_activity"
#save.image("C:/Users/jgolebiowska/Documents/IGB_phd/virus/viral_abundances/mantel_test/mental_test_elbe.RData")
```
...
# dev.off()
tiff("C:/Users/jgolebiowska/Documents/IGB_phd/virus/viral_abundances/mantel_test/mantel_plots/mantel_test_METAG_FL_plot_bray.tiff",width=570,height=300,units="px")
mantel_test_METAG_FL_plot
dev.off()
```
...
# dev.off()
tiff("C:/Users/jgolebiowska/Documents/IGB_phd/virus/viral_abundances/mantel_test/mantel_plots/mantel_test_METAT_FL_plot_bray.tiff",width=520,height=300)
mantel_test_METAT_FL_plot
dev.off()
```
##2. Light fraction
...
motus_expression_LF.dist <- vegdist(t(motus_expression_LF[,-1]), "euclidean")

host_motus_METAG_LF.dist <- vegdist(t(host_motus_METAG_LF[,-1]), "euclidean")
host_motus_expression_LF.dist <- vegdist(t(host_motus_expression_LF[,-1]), na.rm = T, "euclidean")
...
matrix_res_stat_METAT_LF["activity", "MOTUs"] <- matrix_res_stat_METAT_LF["MOTUs","activity"] 
...
matrix_res_sig_METAT_LF["activity", "MOTUs"] <- matrix_res_sig_METAT_LF["MOTUs","activity"] 
...
motus_expression_HF.dist <- vegdist(t(motus_expression_HF[,-1]), "euclidean")

host_motus_METAG_HF.dist <- vegdist(t(host_motus_METAG_HF[,-1]), "euclidean")
host_motus_expression_HF.dist <- vegdist(t(host_motus_expression_HF[,-1]), na.rm = T, "euclidean")
...
library(reshape2)
matrix_res_stat_METAT_HF["activity", "MOTUs"] <- matrix_res_stat_METAT_HF["MOTUs","activity"] 
matrix_res_sig_METAT_HF["activity", "MOTUs"] <- matrix_res_sig_METAT_HF["MOTUs","activity"] 
