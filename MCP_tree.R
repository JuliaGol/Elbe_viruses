...
MCP_annot <- read.csv(file="data/headers_MCP.txt", sep="\t", header=F)
...
#metadata
drep_clusters_fin_taxonomy_unified <- readRDS("data/drep_clusters_fin_taxonomy_unified.RDS")
geneabund_drep_marker_taxonomy_long <- readRDS("data/CCA_marker_gene/geneabund_drep_marker_taxonomy_long.RDS")
...
tree_MCP_E <- read.iqtree(file="data/Combined_phagegenes_E_MCP.aln.contree")
#REFSEQ major capsid protein
dup_MCP <- read.csv(file="data/duplicated.detail_E_MCP.txt", sep='\t', header=F)
...
metadata_params <- read.csv("data/PhysicochemicalParameters_mod3.csv", header=TRUE, sep=",", row.names=1) %>% select(!c("data_type", "sampleid")) %>% distinct()
...
dev.off()
#REFSEQ major capsid protein
dup_MCP <- read.csv(file="data/duplicated.detail_REFSEQ_MCP.txt", sep='\t', header=F)
dup_MCP$V2 <-  gsub("=", "_", dup_MCP$V2)

#separate into vectors
new_dup_tip <-strsplit(dup_MCP$V2, split= ", ", fixed=TRUE)
tree_MCP_REFSEQ <- read.iqtree(file="data/Combined_phagegenes_REFSEQ_MCP.aln.contree")
...
#Probable major capsid protein gp17"
#read duplicated sequences
dup_MCP <- read.csv(file="data/duplicated.detail_gp17_DNA_MCP.txt", sep='\t', header=F)
dup_MCP$V2 <-  gsub("=", "_", dup_MCP$V2)

#separate into vectors
new_dup_tip <-strsplit(dup_MCP$V2, split= ", ", fixed=TRUE)

tree_MCP_gp17 <- read.iqtree(file="data/Combined_phagegenes_gp17_MCP.aln.contree")
#remove without first element (leave one)
for (i in 1:length(new_dup_tip)){
  vec=new_dup_tip[[i]]
...
dup_MCP <- read.csv(file="data/duplicated.detail_euk_DNA_MCP.txt", sep='\t', header=F)
dup_MCP$V2 <-  gsub("=", "_", dup_MCP$V2)

#separate into vectors
new_dup_tip <-strsplit(dup_MCP$V2, split= ", ", fixed=TRUE)
tree_MCP_euk_DNA <- read.iqtree(file="data/Combined_phagegenes_euk_DNA_MCP.aln.contree")
#remove without first element (leave one)
for (i in 1:length(new_dup_tip)){
  vec=new_dup_tip[[i]]
...
IP_hist_all <- readRDS(file="data/IP_hist_all.rds")
IP_hist_MCP <- readRDS(file="data/IP_hist_MCP.rds")
