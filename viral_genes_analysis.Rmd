...
psbA_vir_plot %>%  
  ...
  filter(!is.na(counts)) %>%
  mutate(counts=counts/1000) %>%
    group_by(sampleid, Chlorophyll, data_type) %>% 
  summarise(counts=sum(counts)) %>%
...
psbA_vir_plot %>%  
  ...
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=dCO2_uM, y=counts)) + ggtitle("viral psbA counts across different CO2 values for different data sets") + geom_point()  + scale_y_log10() + geom_tile() + facet_wrap(~data_type, scale = "free_y") + geom_smooth() 
...
psbA_vir_plot %>%  
  ...
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=dCH4_nM, y=counts)) + ggtitle("viral psbA counts across different CH4 values for different data sets") + geom_point()  + scale_y_log10() + geom_tile() + facet_wrap(~data_type, scale = "free_y") + geom_smooth(method = "lm") 
dev.copy(jpeg, filename="C:/Users/jgolebiowska/Documents/IGB_phd/virus/viral_abundances/viral_psbA_CH4.jpg")
dev.off ()
...
psbA_prok_plot %>%  
  ...
  mutate(counts=counts/1000) %>%
    group_by(sampleid, dCH4_nM, data_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=dCH4_nM, y=counts)) + ggtitle("Prokaryotic psbA counts across different CO2 values for different data sets") + geom_point()  + scale_y_log10() + geom_tile() + facet_wrap(~data_type, scale = "free_y") + geom_smooth(method = "lm") 
dev.copy(jpeg, filename="C:/Users/jgolebiowska/Documents/IGB_phd/virus/viral_abundances/viral_psbA_CH4.jpg")
dev.off ()
...
psbA_all_plot <- left_join(psbA_prok_plot[, c("AccessionNumber_TBDSven", "Station","Sample_date", "Stromkilometer", "counts","data_type","Sample_type", "Chla_ug/l")], psbA_vir_plot[, c("AccessionNumber_TBDSven", "counts")], by = "AccessionNumber_TBDSven")
...
psbA_all_plot_origin %>%  
  ...
  filter(Sample_date != "Nov 21") %>%
    group_by(Stromkilometer, Sample_date, Sample_type, data_type, origin) %>% 
  summarise(counts=sum(as.numeric(counts), na.rm = TRUE)) %>%
  ggplot(aes(x=as.numeric(Stromkilometer), y=counts, colour=as.factor(origin)))  + geom_point() + geom_smooth() + ggtitle("viral and prokaryotic psbA counts across different seasons and river sites with Chlorophyll values")  +
  scale_y_log10() + 
  geom_tile() + facet_wrap(~data_type *Sample_date, scale = "free_y") 
```
