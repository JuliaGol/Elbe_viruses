...
pmoC_vir_plot %>% 
  ...
  mutate(counts=counts/1000) %>%
    group_by(sampleid, dCH4_uM, data_type, Sample_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=dCH4_uM, y=counts)) + ggtitle("viral pmoC counts across different CH4 values for different data sets") + geom_point() + geom_tile() + facet_wrap(~data_type *Sample_type, scale = "free_y") + geom_smooth(method = "lm") 
