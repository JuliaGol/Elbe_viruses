...
#big file avoid loading multiple times
#vibrant_annot <- read.csv("C:/Users/jgolebiowska/Documents/IGB_phd/BICEST/virus/viral_abundances/BICEST.phages.annot",  sep = "\t", header = T)
...
##lets load the viral data 
vir_genecluster_annot <- readRDS(file = "vir_genecluster_annot.RDS") %>% select(-c("genome", "range", "DESCRIPTION", "mem",  "length", "pos")) %>% distinct() 
...
metadata <- metadata[,!(colnames(metadata) %in% c("RespirationRate_O2ug.L.h", "DIC_uM.L", "DOC_uM.L", "Total_DIN_uM", "Ammonium_uM", "Nitrite_uM", "Nitrate_uM", "dCH4_uM", "dCO2_nM", "Silicate_uM", "SRP_uM"))]
#ids for expression normalisation - transcription/gene counts
#to numeric
numeric_values <- c("DNA_concentration_ng.uL", "StationNumber", "Stromkilometer", "Sat_O2_Perc", "Temperature_TBDHereon", "Salinity_PSU", "Turbidity_NTU", "pH", "O2_uM","SPM_mgperL", "DOC_mg.L", "TN_mg.L", "DIC_mg.L", "Silicate_mg.L", "Ammonium_mg.L", "Nitrite_mg.L", "Nitrate_mg.L", "SRP_mgperL", "TotalDissolvedPhosphate_mg.L", "Phosphate_uM", "POC_mgperL", "PTC_mgperL", "PTN_mgperL", "PTH_mgperL", "TEP_um2perL", "CSP_um2perL", "ParticleArea_um2perL", "dCH4_nM", "dCO2_uM", "Chlorophyll_Fluoresence",  "Chla_ug/l")
...
#remove contigs ids - as one gene cluster can belong to multiple contigs 
...
#K10945
pmoB_vir <- vir_genecluster_annot %>% filter(KO %in% c("K10945")) #0
#K10946
...
psbA_vir_plot$Turbidity_NTU <- sub(",", ".", psbA_vir_plot$Turbidity_NTU)
psbA_vir_plot$Turbidity_NTU <- as.double(psbA_vir_plot$Turbidity_NTU)
...
psbA_vir_plot %>%  
  ...
  filter(!is.na(counts)) %>%
  mutate(counts=counts/1000) %>%
    group_by(sampleid, Turbidity_NTU, Sample_date, data_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=Turbidity_NTU, y=counts)) + ggtitle("viral psbA counts across different seasons and river sites") + geom_point()  + scale_y_log10() +
  geom_tile() + facet_wrap(~data_type *Sample_date, scale = "free_y") + geom_smooth() 
...
psbA_vir_plot %>%  
  ...
  filter(!is.na(counts)) %>%
  mutate(counts=counts/1000) %>%
    group_by(sampleid, Turbidity_NTU, data_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=Turbidity_NTU, y=counts)) + ggtitle("viral psbA counts across different Turbidity values across different data sets") + geom_point()  + scale_y_log10() +
...
psbA_vir_plot %>%  
  ...
  filter(!is.na(counts)) %>%
  mutate(counts=counts/1000) %>%
    group_by(sampleid, `Chla_ug/l`, data_type, Sample_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=`Chla_ug/l`, y=counts)) + ggtitle("viral psbA counts across different chlorophyll values across different data sets") + geom_point()  + scale_y_log10() +
...
dev.copy(jpeg, filename="C:/Users/jgolebiowska/Documents/IGB_phd/BICEST/virus/viral_abundances/viral_psbA_CO2.jpg")
dev.off ()
...
psbA_vir_plot %>%  
  ...
  filter(!is.na(counts)) %>%
  mutate(counts=counts/1000) %>%
    group_by(sampleid, dCH4_uM, data_type, Sample_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=dCH4_uM, y=counts)) + ggtitle("viral psbA counts across different CH4 values for different data sets") + geom_point()  + scale_y_log10() + geom_tile() + facet_wrap(~data_type *Sample_type, scale = "free_y") + geom_smooth(method = "lm") 
dev.copy(jpeg, filename="C:/Users/jgolebiowska/Documents/IGB_phd/virus/viral_abundances/viral_psbA_CH4.jpg")
dev.off ()
...
psbA_vir_plot$Salinity_PSU <- sub(",", ".", psbA_vir_plot$Salinity_PSU)
psbA_vir_plot$Salinity_PSU <- as.double(psbA_vir_plot$Salinity_PSU)
...
psbA_vir_plot %>%  
  ...
  filter(!is.na(counts)) %>%
  mutate(counts=counts/1000) %>%
    group_by(sampleid, Salinity_PSU, Sample_date, data_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=Salinity_PSU, y=counts)) + ggtitle("viral psbA counts across different seasons and river sites") + geom_point()  + scale_y_log10() +
  geom_tile() + facet_wrap(~data_type *Sample_date, scale = "free_y") + geom_smooth() 
...
```{r}
petE_vir_plot$Turbidity_NTU <- sub(",", ".", petE_vir_plot$Turbidity_NTU)
petE_vir_plot$Turbidity_NTU <- as.double(petE_vir_plot$Turbidity_NTU)
...
petE_vir_plot %>%  
  ...
  mutate(counts=counts/1000) %>%
    group_by(sampleid,Turbidity_NTU, Sample_date, data_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=Turbidity_NTU, y=counts)) + ggtitle("viral petE counts across different seasons and river sites") + geom_point()  + scale_y_log10() +
  geom_tile() + facet_wrap(~data_type *Sample_date, scale = "free_y") + geom_smooth() 

```

```{r}
petE_vir_plot$Salinity_PSU <- sub(",", ".", petE_vir_plot$Salinity_PSU)
petE_vir_plot$Salinity_PSU <- as.double(petE_vir_plot$Salinity_PSU)
...
petE_vir_plot %>%  
  ...
  mutate(counts=counts/1000) %>%
    group_by(sampleid,Salinity_PSU, Sample_date, data_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=Salinity_PSU, y=counts)) + ggtitle("viral petE counts across different seasons and river sites") + geom_point()  + scale_y_log10() +
  geom_tile() + facet_wrap(~data_type *Sample_date, scale = "free_y") + geom_smooth() 
...
#dates order
psbA_all_plot$Sample_date <-  factor(psbA_all_plot$Sample_date, levels=c("May-21", "Jul-21", "Feb-22","May-22", "Jun-22", "Nov-22"))
...
psbA_all_plot <- data.frame(psbA_all_plot)
tiff("viral_to_prokaryotic_psbA_counts_ratios_across_spatio-temporal_scales.tiff", unit="cm", height=21.0, width=29.7, res =300)
psbA_vtop <- psbA_all_plot %>%  
  #filter(Station != 'BunthausSpitze') %>% 
  filter(Sample_date != "Nov 21") %>%
    #group_by(Stromkilometer, Sample_date, Sample_type, data_type, `Chla_ug/l`) %>% 
  ggplot(aes(x=as.numeric(Stromkilometer), y=vtop_ratio))  + geom_point() + geom_smooth() + facet_wrap(~data_type *Sample_date, scales = "free_y") + geom_tile() + scale_y_log10() + xlab("Elbe km") + ylab("viral to prokaryotic psbA counts") + theme_bw()
psbA_vtop
dev.off()
pdf("viral_to_prokaryotic_psbA_counts_ratios_across_spatio-temporal_scales.pdf",height=21.0/2, width=29.7)
psbA_vtop
dev.off()
#salinity
tiff("viral_to_prokaryotic_psbA_counts_ratios_against_salinity.tiff")
psbA_all_plot %>%   filter(Sample_date != "Nov 21") %>%  ggplot(aes(x=Salinity_PSU, y=vtop_ratio))  + geom_point() + geom_smooth(method="lm")  + ggtitle("viral to prokaryotic psbA counts ratios against salinity") + geom_tile() + facet_wrap(~data_type, scales = "free_y") #+ geom_point(aes(x=as.numeric(Salinity_PSU), y=`Chla_ug/l`), fill="green", colour="green") + geom_smooth(aes(x=as.numeric(Salinity_PSU), y=`Chla_ug/l`), colour="green")
dev.off()
...
psbA_prok_plot %>%  
  ...
  filter(!is.na(counts)) %>%
  mutate(counts=counts/1000) %>%
    group_by(sampleid, dCH4_uM, data_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=dCH4_uM, y=counts)) + ggtitle("Prokaryotic psbA counts across different CH2 values for different data sets") + geom_point()  + scale_y_log10() + geom_tile() + facet_wrap(~data_type, scale = "free_y") + geom_smooth(method = "lm") 
...
psbA_all_plot_origin %>%  
  filter(Station != 'BunthausSpitze') %>% 
  filter(Sample_date != "Nov 21") %>%
    group_by(Stromkilometer, Sample_date, Sample_type, data_type, origin, `Chla_ug/l`) %>% 
  summarise(counts=sum(as.numeric(counts), na.rm = TRUE)) %>%
  ggplot(aes(x=as.numeric(Stromkilometer), y=counts, fill=as.factor(origin)))  + geom_point() + geom_smooth() + geom_point(aes(x=as.numeric(Stromkilometer), y=`Chla_ug/l`), fill="green", colour="green") + geom_smooth(aes(x=as.numeric(Stromkilometer), y=`Chla_ug/l`), colour="green") + ggtitle("viral and prokaryotic psbA counts across different seasons and river sites with Chlorophyll values")  +
  scale_y_log10() + 
...
cor(psbA_all_plot_origin_vir$counts,psbA_all_plot_origin_vir$`Chla_ug/l`, use='pairwise.complete.obs')
cor(psbA_all_plot_origin_host$counts,psbA_all_plot_origin_host$`Chla_ug/l`, use='pairwise.complete.obs')
#correlation near 0
...
AlaS_all_plot$Sample_date <-  factor(AlaS_all_plot$Sample_date, levels=c("May-21", "Jul-21", "Nov-21", "Feb-22","May-22", "Jun-22",  "Nov-22"))
...
tiff("viral_to_prokaryotic_AlaS_counts_ratios_across_spatio-temporal_scales.tiff", unit="cm", height=21.0, width=29.7, res =300)
AlaS_vtop <- AlaS_all_plot %>% filter(Sample_date != "Nov 21") %>% ggplot(aes(x=as.numeric(Stromkilometer), y=vtop_ratio))  + geom_point() + geom_smooth() + geom_tile() + facet_wrap(~data_type *Sample_date, scales = "free_y") + scale_y_log10() + xlab("Elbe km") + ylab("viral to prokaryotic alaS counts") + theme_bw()
AlaS_vtop
dev.off()
pdf("viral_to_prokaryotic_AlaS_counts_ratios_across_spatio-temporal_scales.pdf", height=21.0/2, width=29.7/2)
AlaS_vtop
dev.off()
#salinity
...
PmoC
```{r CH4}
pmoC_vir_plot <- pmoC_vir %>% 
  pivot_longer(!colnames(pmoC_vir)[c(1:2)], names_to = "sampleid", values_to = "counts", values_drop_na = 0) %>%
  left_join(metadata, by="sampleid")
...
pmoC_vir_plot %>% 
  ...
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=dCH4_nM, y=counts)) + ggtitle("viral pmoC counts across different CH4 values for different data sets") + geom_point() + geom_tile() + facet_wrap(~data_type , scale = "free_y") + geom_smooth(method = "lm") + scale_y_log10()

pmoC_vir_plot %>% 
  filter(Sample_date != "Nov 21") %>%
  filter(!is.na(counts)) %>%
  mutate(counts=counts/1000) %>%
    group_by(sampleid, O2_uM, data_type, Sample_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=O2_uM, y=counts)) + ggtitle("viral pmoC counts across different O2 values for different data sets") + geom_point() + geom_tile() + facet_wrap(~data_type *Sample_type, scale = "free_y") + geom_smooth(method = "lm") 


pmoC_vir_plot %>%  
  #filter(Station != 'BunthausSpitze') %>% 
  filter(Sample_date != "Nov 21") %>%
    group_by(Stromkilometer, Sample_date, Sample_type, data_type) %>% 
  summarise(counts=sum(as.numeric(counts), na.rm = TRUE)) %>%
  ggplot(aes(x=as.numeric(Stromkilometer), y=counts)) + geom_point(aes(colour=as.factor(origin))) + geom_smooth() + 
  ggtitle("Viral and prokaryotic pmoC counts across different seasons and river sites") + scale_y_log10() + xlab("Kilometer") + geom_tile() + facet_wrap(~data_type *Sample_date, scale = "free_y") + labs(fill="Origin")
