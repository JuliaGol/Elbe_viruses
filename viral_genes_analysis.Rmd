...
```{r loading}
# #uncomment if you want to generate an object
# #gene_cluster_table
...
metadata_more <- read.csv("data/Stations_Daten_RTG_250923.csv", sep=",", skip=2, header = F)
...
# saveRDS(psbA_prok, file = "data/psbA_prok.RDS")
#or load object
psbA_prok = readRDS(file = "data/psbA_prok.RDS")
...
#uncomment if you want to generate object 
...
##or lets load the viral data 
vir_genecluster_annot <- readRDS(file = "data/vir_genecluster_annot.RDS") %>% select(-c("genome", "range", "DESCRIPTION", "mem",  "length", "pos")) %>% distinct() 
...
```{r}
AlaS_prok_long <- readRDS(file = "data/meta_AlaS_clust.RDS")
...
AlaS_all_plot %>%  
  ...
  ggplot(aes(x=Salinity_PSU, y=vtop_ratio))  + geom_point() + geom_smooth(method="lm")  + ggtitle("viral to prokaryotic alaS counts ratios against salinity") + geom_tile() + facet_wrap(~data_type, scales = "free_y") #+ geom_point(aes(x=as.numeric(Salinity_PSU), y=`Chla_ug/l`), fill="green", colour="green") + geom_smooth(aes(x=as.numeric(Salinity_PSU), y=`Chla_ug/l`), colour="green")
dev.off()
...
AlaS_all_plot %>%  
  ...
  filter(Sample_date != "Nov 21") %>%   group_by(data_type, Sample_type, Stromkilometer, Sample_date) %>% 
    summarise(viral=sum(viral, na.rm = TRUE), prokaryotic=sum(prokaryotic,na.rm = TRUE)) %>%
  ggplot(aes(x=prokaryotic, y=viral)) + ggtitle("viral alaS against prokaryotic alaS counts") + geom_point()  + scale_y_log10() +
  geom_tile() + facet_wrap(~data_type , scale = "free") + geom_smooth() 
dev.off()
```
...
AlaS_all_plot %>%  
  ...
  filter(data_type == "METAG") %>%
    group_by(Stromkilometer, data_type, Sample_type, Sample_date) %>% 
  summarise(viral=sum(viral, na.rm = TRUE), prokaryotic=sum(prokaryotic, na.rm = TRUE)) %>%
  ggplot(aes(x=prokaryotic, y=viral)) + ggtitle("viral and prokaryotic alaS counts across different seasons and river sites") + geom_point()  + scale_y_log10() +
