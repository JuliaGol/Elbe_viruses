...
#bubble plot version
mm =  vir_genecluster_annot_pathway_salinity_level %>% ggplot(aes(x = sample_spatio_temp, y=Metabolism)) + 
 ...
 # scale_fill_manual(values = colours, guide = FALSE) + 
  ...
  legend.title = element_text(size = 12, face = "bold"), strip.text.x = element_text(size = 10),
  panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),  legend.position = "bottom") + guides(color = "none")
tiff("metabolism_salinity_level_bubble_plot.tiff", unit="cm", height=35, width=60, res =300)
mm
dev.off()
pdf("metabolism_salinity_level_bubble_plot.pdf", height=21.0, width=29.7)
mm
dev.off()
```
```{r}
