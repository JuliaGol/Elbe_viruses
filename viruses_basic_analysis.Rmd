...
#Prepare data
#old file to compare
pathways <- read.csv("C:/Users/jgolebiowska/Documents/IGB_phd/BICEST/virus/Elbe_viruses_distribution/VIBRANT_results_Combined_viruses/VIBRANT_AMG_pathways_Combined_viruses.tsv" , sep='\t' , header = F)
...
#to save computations remove unneccessery columns
vir_genecluster_annot <- vir_genecluster_annot %>% select(-c("range", "DESCRIPTION", "mem",  "length", "pos")) %>% merge(pathways[,c("Metabolism", "Pathway", "Present.AMG.KOs")], by.x="KO", by.y="Present.AMG.KOs") # find more pathways 
#add drep clusters 
#but first unify genome names
vir_genecluster_annot <- vir_genecluster_annot %>% 
  mutate(genome=str_remove(gene, "_[0-9]+$")) %>% group_by(Metabolism, Pathway) %>% summarise(across(where(is.numeric), sum))
...
pathways_fractions_plot <- vir_genecluster_annot %>%  ggplot() + geom_bar(position ="fill", stat = "identity", aes(x=reorder(sampleid, StationNumber), y=counts, fill = Pathway)) + facet_wrap(~data_type *Sample_type, scales= "free_x",labeller = as_labeller(new_labels)) + scale_y_continuous(labels=scales::percent_format()) + scale_x_discrete(labels = NULL, breaks = NULL) + ggtitle("Relative abundace of pathways across different fractions") + ylab("Relative abundace") + xlab("") + theme(axis.title.y = element_text(angle=90, size=12)) +  theme_bw(base_size=20)
tiff("Pathways_fractions_plot.tiff", unit="cm", height=29.7, width=21.0, res =300)
pathways_fractions_plot
dev.off()
#fractions metabolism
metabolism_fractions_plot <- vir_genecluster_annot %>%  ggplot() + geom_bar(position ="fill", stat = "identity", aes(x=reorder(sampleid, StationNumber), y=counts, fill = Metabolism)) + facet_wrap(~data_type *Sample_type, scales= "free_x",labeller = as_labeller(new_labels)) + scale_y_continuous(labels=scales::percent_format()) + scale_x_discrete(labels = NULL, breaks = NULL) + ggtitle("Relative abundace of metabolism across different fractions") + ylab("Relative abundace") +xlab("")+ theme( legend.position="bottom", axis.title.y = element_text(angle=90))  +  theme_bw(base_size=15)
...
tiff("Metabolism_fractions_plot.tiff", unit="cm", height=29.7, width=21.0, res =300)
metabolism_fractions_plot
dev.off()
pdf("Metabolism_fractions_plot.pdf", height=29.7, width=21.0)
metabolism_fractions_plot
dev.off()
#sampling date
...
pathway_salinity_plot <-  vir_genecluster_annot_pathway_salinity_level %>% ggplot(aes(x=sample_spatio_temp, y = total_Percentage, fill = Pathway)) + geom_bar(stat='identity',  position = "fill") + scale_y_continuous(labels=scales::percent_format())  + xlab("Elbe km") + ylab("Relative abundance") + scale_fill_manual(values = palette_66_contrast)+ facet_grid2(data_type ~ Salinity_level, render_empty=F, scales="free", independent = "x") + theme_clean(base_size=18) + theme(axis.title.y = element_text(angle=90), axis.text.x = element_text(angle=90), strip.text.x = element_text(size = 15), legend.position = "bottom", legend.background = element_rect(color = NA),  legend.key.size =  unit(0.5, "cm"))  + scale_x_discrete(name ="Sample") + guides(fill=guide_legend(title.position = "top", ncol=4))

tiff("pathway_salinity_plot.tiff", unit="cm", width=29.7, height=21.0, res =300)
pathway_salinity_plot
dev.off()
...
tiff("metabolism_salinity_plot.tiff", unit="cm", width=29.7, height=21.0, res =300)
metabolism_salinity_plot
dev.off()
...
#bubble plot version
xx =  vir_genecluster_annot_pathway_salinity_level %>% ggplot(aes(x = sample_spatio_temp, y=Pathway)) + 
 ...
 # scale_fill_manual(values = colours, guide = FALSE) + 
  scale_y_discrete(limits = rev(levels(vir_genecluster_annot_pathway_salinity_level$Pathway))) +
...
vibrant_genecluster_abund_quality <- vibrant_genecluster_abund %>% merge(Combined_quality_summary) %>% merge(drep_clusters_fin_taxonomy_unified, by="primary_cluster")
#analyse complete
vibrant_genecluster_abund_quality_complete <- vibrant_genecluster_abund_quality[which(vibrant_genecluster_abund_quality$checkv_quality=="Complete"),]
#check the taxonomy
unique(vibrant_genecluster_abund_quality_complete$level_5)
# "Caudoviricetes" only
vibrant_genecluster_abund_quality_hq <- vibrant_genecluster_abund_quality[which(vibrant_genecluster_abund_quality$checkv_quality=="High-quality"),]
#check the taxonomy complete
unique(vibrant_genecluster_abund_quality_complete$level_5)
# "Caudoviricetes" only
#check the taxonomy high quality 
unique(vibrant_genecluster_abund_quality_hq$level_5)
# "Caudoviricetes" only
#pathways 
#check the taxonomy complete
unique(vibrant_genecluster_abund_quality_complete$Pathway)
# "Caudoviricetes" only
#check the taxonomy high quality 
unique(vibrant_genecluster_abund_quality_hq$Pathway)
#check carbon fixation pathway in potosyyntetic organisms - are those
#cyanophages?
vibrant_genecluster_abund_quality_photo <- vibrant_genecluster_abund_quality[which((vibrant_genecluster_abund_quality$checkv_quality=="High-quality" | vibrant_genecluster_abund_quality$checkv_quality=="Complete") & vibrant_genecluster_abund_quality$Pathway=="Carbon fixation in photosynthetic organisms"),] %>% merge(host_prediction_genome)

# "Caudoviricetes" only
drep_clusters_fin_taxonomy_unified <- drep_clusters_fin_taxonomy_unified %>% filter(!is.na(Salinity_niche))
vibrant_genecluster_abund_niche <- merge(vibrant_genecluster_abund, drep_clusters_fin_taxonomy_unified, by="primary_cluster")

#add quality 
salinity_niche_pathways_plot <- vibrant_genecluster_abund_niche  %>% filter(counts>0 & !is.na(Sample_date)) %>% group_by(sampleid, Stromkilometer, Salinity_niche, data_type, Pathway) %>% summarise(counts=sum(counts)) %>% ggplot() + geom_bar(position ="fill", stat = "identity", aes(x=as.factor(round(Stromkilometer,0)), y=counts, fill = Pathway)) + facet_wrap(~data_type *Salinity_niche, scales= "free_x") + scale_y_continuous(labels=scales::percent_format()) + scale_y_continuous(labels=scales::percent_format()) + ggtitle("Relative abundace of pathways across different fractions") + ylab("Relative abundace") + theme(axis.title.y = element_text(angle=90, size=12), legend.text = element_text(size=10)) + xlab("Elbe km") + scale_fill_manual(values = palette)
...
Fraction_niche_pathways_plot <- vibrant_genecluster_abund_niche  %>% filter(counts>0 & !is.na(Sample_date)) %>% group_by(sampleid, Stromkilometer, Fraction_niche, data_type, Pathway) %>% summarise(counts=sum(counts)) %>% ggplot() + geom_bar(position ="fill", stat = "identity", aes(x=reorder(sampleid, Stromkilometer), y=counts, fill = Pathway)) + facet_wrap(~data_type *Fraction_niche, scales= "free_x") + scale_y_continuous(labels=scales::percent_format()) + scale_y_continuous(labels=scales::percent_format()) + ggtitle("Relative abundace of pathways across different fractions") + ylab("Relative abundace") + theme(axis.title.y = element_text(angle=90, size=12), legend.text = element_text(size=10)) + xlab("Elbe km") + scale_fill_manual(values = palette)

Fraction_niche_pathways_plot <- vibrant_genecluster_abund_niche  %>% filter(counts>0 & !is.na(Sample_date)) %>% ggplot() + geom_bar(position ="fill", stat = "identity", aes(x=Fraction_niche, y=counts, fill = Pathway)) + scale_y_continuous(labels=scales::percent_format()) + scale_y_continuous(labels=scales::percent_format()) + ylab("Relative abundace") + theme(axis.title.y = element_text(angle=90, size=12), legend.text = element_text(size=10)) + xlab("Elbe km") + scale_fill_manual(values = palette)
