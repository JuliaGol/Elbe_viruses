...
annot <- read.csv("GROS22.kegg-annotations.tsv", sep="\t", skip=0, header = TRUE, row.names = 1, quote="")
clstr <- read.csv("data/GROS22.clstr", sep="\t", skip=0, header = FALSE)
...
vibrant_annot <- read.csv("data/BICEST.phages.annot", header = FALSE, sep = "\t")
colnames(vibrant_annot) <- c("gene", "range", "KO", "DESCRIPTION")

##Load Metadata for later

metadata <- read.csv("data/SAMEAID_SampleID_simplified.csv", header=TRUE, sep=";") %>% 
  mutate(sampleid=paste0(ProjectID,"_",BioSample,"_METAT.genecount.profile")) %>%
  mutate(data_type="METAT")


metadata2 <- read.csv("data/SAMEAID_SampleID_simplified.csv", header=TRUE, sep=";") %>% 
...
saveRDS(vir_genecluster_annot, "data/vir_genecluster_annot.rds")
