...
date: "2024-03-14"
output: pdf_document
#only METAT data makes sens as they how expession ber cell - when METAG shouw counts pers cell - what whould be more interesting to have total abundance - multiply by MoTu? Where is original file??????????
---
...
```{r}
meta_rhd_clust$Salinity_TBDHereon <- sub(",", ".", meta_rhd_clust$Salinity_TBDHereon)
meta_rhd_clust$Salinity_TBDHereon <- as.double(meta_rhd_clust$Salinity_TBDHereon)
#plot it!
meta_rhd_clust %>%  
  filter(Station != 'BunthausSpitze') %>% 
  filter(Sample_date != "Nov 21") %>%
  mutate(counts=counts/1000) %>%
  group_by(sampleid,Sample_date,Salinity_TBDHereon, data_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=Salinity_TBDHereon, y=counts)) + geom_point() +
...
meta_rhd_clust %>%  
  ...
  filter(Sample_date != "Nov 21") %>%
  mutate(counts=counts/1000) %>%
  group_by(sampleid, Salinity_TBDHereon, Sample_date, Sample_type) %>% 
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=Salinity_TBDHereon, y=counts)) + geom_point() +
  geom_tile() + facet_wrap(~ Sample_type, scales="free_y")  + geom_smooth(method = 'loess', formula= y ~ x)
```
...
meta_rhd_clust %>% 
  ...
  ggplot(aes(x=Salinity_TBDHereon, y=counts)) + geom_point() + geom_smooth(method = 'lm', formula= y ~ x) + 
 facet_wrap(~ data_type, scales="free_y")
#pH

meta_rhd_clust$pH_TBDHereon <- sub(",", ".", meta_rhd_clust$pH_TBDHereon)
meta_rhd_clust$pH_TBDHereon <- as.double(meta_rhd_clust$pH_TBDHereon)
  meta_rhd_clust %>% 
  filter(Station != 'BunthausSpitze') %>% 
  filter(Sample_date != "Nov 21") %>%
  group_by(sampleid,Stromkilometer, Sample_date, data_type) %>% 
  summarise(counts=sum(counts), pH_TBDHereon ) %>%
ggplot( aes(x=pH_TBDHereon, y=counts)) + geom_point()  + geom_smooth(method = 'lm', formula= y ~ x) +
    facet_wrap(~ data_type, scales="free_y") #ommit rows with zero - artifact from NA <= 0- artifact from NA <= 0
#tubidity
  meta_rhd_clust$Turbidity_TBDHereon <- sub(",", ".", meta_rhd_clust$Turbidity_TBDHereon)
meta_rhd_clust$Turbidity_TBDHereon <- as.double(meta_rhd_clust$Turbidity_TBDHereon)
meta_rhd_clust %>% 
  filter(Station != 'BunthausSpitze') %>% 
  filter(Sample_date != "Nov 21") %>%
  group_by(sampleid, Stromkilometer, Sample_date, data_type) %>% 
  summarise(counts=sum(counts), Turbidity_TBDHereon) %>%
ggplot(aes(x=Turbidity_TBDHereon, y=counts)) + geom_point()  + geom_smooth(method = 'lm', formula= y ~ x) +
    facet_wrap(~ data_type, scales="free_y")  #ommit rows with zero 
...
Check other phototrophy genes: listed in sunegawa paper - Gene Expression Changes and Community Turnover
...
denitrification:
napA - K02567, nirS - K15864, norB - K04561, and nosZ - K00376
Transcripts of nitrogen fixation marker genes - nifK,
nifH - K02588, and nifD - K02586
```{r}
#Check if they are present  the data:
  #find psbA genes 
psaA_clust <- annot %>%  
  filter(KO %in% c("K02689")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
...
#(petC - K02636, petE - K02638, and petH - K02641) and the cyanobacteria-specific antenna proteins
petC_clust <- annot %>%  
  filter(KO %in% c("K02636")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
petE_clust <- annot %>%  
  filter(KO %in% c("K02638")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
petH_clust <- annot %>%  
  filter(KO %in% c("K02641")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
#(apcA - K02092, apcF - K02097, cpcA - K02284, cpeA - K05376, and cpeT - K05383)
apcA_clust <- annot %>%  
  filter(KO %in% c("K02092")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
apcF_clust <- annot %>%  
  filter(KO %in% c("K02097")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
cpcA_clust <- annot %>%  
  filter(KO %in% c("K02284")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
cpeA_clust <- annot %>%  
  filter(KO %in% c("K05376")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
cpeT_clust <- annot %>%  
  filter(KO %in% c("K05383")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
...
nifH_clust <- annot %>%  
  ...
  left_join(geneabund,  join_by(QUERY == gene_cluster))
nifD_clust <- annot %>%  
  filter(KO %in% c("K02586")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
#denitrification:
napA_clust <- annot %>%  
  filter(KO %in% c("K02567")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
nirS_clust <- annot %>%  
  filter(KO %in% c("K15864")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
norB_clust <- annot %>%  
  filter(KO %in% c("K04561")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
nosZ_clust <- annot %>%  
  filter(KO %in% c("K00376")) %>%
  left_join(geneabund,  join_by(QUERY == gene_cluster))
```
Explore spatial and temporal changes 
```{r}
psaA_clust <- psaA_clust %>% 
  pivot_longer(!c("QUERY", "KO", "BRITE", "CAZY", "COG", "DISEASE", "DRUG", "ENZYME", "GO", "MODULE", "NETWORK", "PATHWAY", "PUBMED", "RCLASS", "REACTION", "TC", "VP", "DESCRIPTION", "length"), names_to = "sampleid", values_to = "counts", values_drop_na = 0) %>%
  left_join(metadata, by="sampleid" )
psaA_clust %>%  
  filter(Station != 'BunthausSpitze') %>% 
  ...
  summarise(counts=sum(counts)) %>%
  ggplot(aes(x=Stromkilometer, y=counts)) + geom_point() +
  geom_tile() + facet_wrap(~ Sample_date, *Stromkilometer, scale = "free_y") + geom_smooth()
```
