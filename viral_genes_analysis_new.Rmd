...
library(tidyverse)
library(ggplot2)
setwd("~/IGB_phd/BICESTvirus/viral_genes")
...
metadata <-metadata %>% 
  ...
  mutate(Sample_date=str_replace(Sample_date, "Mai", "May"))
...
```{r}
psbA_all_plot <- left_join(psbA_prok_plot[, c("AccessionNumber_TBDSven", "Station","Sample_date", "Stromkilometer", "counts","data_type","Sample_type", "Chlorophyll")], psbA_vir_plot[, c("AccessionNumber_TBDSven", "counts", "gene_cluster")], by = "AccessionNumber_TBDSven")
...
```
```{r}
path = "C:/Users/jgolebiowska/Documents/IGB_phd/virus/viral_genes/"
