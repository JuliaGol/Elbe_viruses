...
log_expression_LF <- log_expression_LF + abs(min(log_expression_LF)) #add absolute value of minimum to get positive values - not sure if it is correct 

log_expression_LF <- as.data.frame(log_expression_LF)

rownames(log_expression_LF) <- gsub("GROS22.(1|2)_", "", rownames(log_expression_LF))
rownames(log_expression_LF) <- gsub("_METAT.genecount.profile", "", rownames(log_expression_LF))

cca_expression_LF <- cca(as.matrix(log_expression_LF), as.matrix(metadata_METAT_LF_filled))
...
plot = ggplot(data = veg_2_expr_LF[which(veg_2_expr_LF$Metabolism=="Amino acid metabolism" &  veg_2_expr_LF$Total.AMGs > 10),]) +
  geom_point(aes(x = CCA1, y = CCA2, colour = Pathway)) + geom_mark_ellipse(aes(x = CCA1, y = CCA2, color=Pathway)) # + theme(legend.position="none")
#+ geom_point(data = veg_1, aes(x = CCA1, y = CCA2), color = "blue")
...
selected_pathways <-c("Phosphonate and phosphinate metabolism", "Lipoarabinomannan (LAM) biosynthesis", "Glycosaminoglycan degradation", "Retinol metabolism")
plot = ggplot() +
  geom_point(data = veg_2_expr_LF[which(veg_2_expr_LF$Pathway %in% selected_pathways),], aes(x = CCA1, y = CCA2, colour = Pathway))
plot
beep(sound = 1, expr = NULL)
```
Not informative - focus on gene abundance in the othe fraction
