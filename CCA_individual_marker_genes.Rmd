...
r2 <- summary(lm_pred)$r.squared
model_temp_validation<-ggplot() + geom_point(aes(x=virusTest$Temperature_TBDHereon, y=pred)) + geom_abline(color="red", size=1) + geom_smooth(aes(x=virusTest$Temperature_TBDHereon, y=pred), method = "lm", se = FALSE) +  annotate(geom="text", x=8, y=20, label=paste("Rsquared = ",round(r2,2)), size= 10, colour = "#3366FF") + ylab("Predicted") + xlab("Observed") + ylim(c(0,25)) + theme(axis.title.x = element_text(size = 20),
        axis.title.y = element_text(size = 20))
