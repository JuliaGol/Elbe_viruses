...
#  scale_y_discrete(limits = rev(levels(geneabund_drep_marker_taxonomy_long$level_5)))
  ...
  panel.background = element_blank(), panel.border = element_rect(colour = "black", fill = NA, size = 1.2),  legend.position = "bottom") + guides(color = guide_legend(override.aes = list(size=5)))
xx
```
...
library(ggpubr)
#get votus data 
geneabund_drep_marker_taxonomy_wide <-  geneabund_drep_marker_taxonomy_long %>% ungroup() %>% filter(Associatednumber%%2==0) %>% select(-c("lineage", "level_5"))  %>%  pivot_wider(names_from=primary_cluster, values_from=counts, values_fill = 0, names_prefix="cluster_") %>% distinct()
...
#beta diversity
pcoa = ordinate(phyloseq2, "PCoA", "bray") 
#as factor Stromkilometer
#unify
phyloseq2@sam_data$Stromkilometer <- round(phyloseq2@sam_data$Stromkilometer)
phyloseq2@sam_data$Stromkilometer[which(phyloseq2@sam_data$Stromkilometer==715)] <- 712 
phyloseq2@sam_data$Stromkilometer[which(phyloseq2@sam_data$Stromkilometer==692)] <- 694 
phyloseq2@sam_data$Stromkilometer[which(phyloseq2@sam_data$Stromkilometer==666)] <- 665 
phyloseq2@sam_data$Stromkilometer[which(phyloseq2@sam_data$Stromkilometer==652)] <- 651
phyloseq2@sam_data$Stromkilometer <- as.factor(phyloseq2@sam_data$Stromkilometer)  
color_colors<- c("black", colors[2:4])
beta_diver <- plot_ordination(phyloseq2, pcoa, color = "Salinity_level",  shape="Stromkilometer") +   geom_point(size = 5, stroke=2, aes(fill = Salinity_level)) +  scale_color_manual(values = color_colors, name = "Salinity")+  scale_fill_manual(values = colors, name = "Salinity")    +  scale_shape_manual(values = c(21,22,23,2,3,4), name= "Elbe km") +  theme_bw(base_size=20) + guides(shape =guide_legend(title = "Elbe km", override.aes = list(color = "black"))) #+  guides(shape = FALSE))

library(ggdark)
#calculate permanova
phyloseq2_bray <- phyloseq::distance(phyloseq2, method = "bray")

# make a data frame from the sample_data
sampledf <- data.frame(sample_data(phyloseq2))

# Adonis test
adonis_station_otus <- adonis2(phyloseq2_bray ~ Station, data = sampledf)
adonis_salinity_otus <- adonis2(phyloseq2_bray ~ Salinity_level, data = sampledf)
...
row_2 <- plot_grid(beta_diver, venn_salinity, rel_widths = c(1,1),labels = c('B', 'C'),label_size	
=18)
tiff("diversity_meassures.tiff", unit="px", width = 1000, height = 1000)
plot_grid(alpha_diver,  row_2,labels = c('A'), ncol=1, rel_widths = c(1,1),label_size	
=18)
dev.off()
svg("diversity_meassures.svg", width = 19, height = 11)
plot_grid(alpha_diver,  row_2,labels = c('A'), ncol=1, rel_widths = c(1,2),label_size	=18)
dev.off()
```
