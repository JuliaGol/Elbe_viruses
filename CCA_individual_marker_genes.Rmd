...
#debug
fresh <- drep_clusters_fin_taxonomy_unified_stat$primary_cluster[which(grepl("fresh", drep_clusters_fin_taxonomy_unified_stat$Salinity_niche))]
...
# Chart
venn.diagram(
  x = list( freshwater = fresh, oligohaline = oligo, mesohaline = meso, polyhaline = poly),
  category.names = c("Freshwater", "Oligohaline" , "Mesohaline" , "Polyhaline"),
  filename = 'salinity_venn_diagramm_blue.png',
  output=TRUE,
  fill = colors,
  col = colors)
...
venn.diagram(
  x = list( freshwater = fresh, oligohaline = oligo, mesohaline = meso, polyhaline = poly),
  category.names = c("Freshwater", "Oligohaline" , "Mesohaline" , "Polyhaline"),
  filename = 'salinity_venn_diagramm_blue.tiff',
  output=TRUE,
  fill = colors,
  col = colors)
...
CCA1_imp <-cca_diversity_FL_sum$cont$importance[2,1]*100
CCA2_imp <-cca_diversity_FL_sum$cont$importance[2,2]*100
```
```{r}
...
```
```{r}
p <- ord_plot(cca_diversity_FL,  env_data =  metadata_METAG_FL_filled, species =F, envfits = c("Salinity_PSU"), fill=veg_2$class, sp_arrow=F, sp_symbol=19)
```
```{r}
ggord_plot(cca_diversity_FL)
...
```{r}
plot = ggplot() +
  geom_point(data = veg_2, aes(x = CCA1, y = CCA2, colour = Salinity_niche), alpha=1)  + scale_color_manual(values=c(brewer.pal(n = 9, name = "Blues"), "yellow", "purple"))
...
for (i in 0:20) {
  ...
  models[[name]] <- glmnet(as.matrix(geneabund_drep_marker_taxonomy_wide_METAG_train_x), y_train, alpha=i/20)
  
}


#test models
results <- data.frame(colnames=c("alpha", "mse",  "mae", "mape", "rsq"))
for (i in 0:20) {
  name <- paste0("alpha", i/20)
  
  ## Use each model to predict 'y' given the Testing dataset
  predicted <- predict(models[[name]], 
                       s=models[[name]][[1]],     newx=as.matrix(geneabund_drep_marker_taxonomy_wide_METAG_test_x))
  ...
  temp <- c(alpha=i/20, mse = mse,  mae = mae, mape = mape, rsq=rsq)
  results <- rbind(results, temp)
}
```
```{r}

predicted_y <- predict(models[["alpha0.25"]], newx=as.matrix(geneabund_drep_marker_taxonomy_wide_METAG_test_x))
plot(y_test, predicted_y[,99], ylim=c(0,20))
...
train_control <- trainControl(method = "cv",
                              ...
                              allowParallel = FALSE)
# #take 
# geneabund_drep_marker_taxonomy_wide_METAG_salinity <-  geneabund_drep_marker_taxonomy_wide_METAG %>% select(!nearZeroVar(geneabund_drep_marker_taxonomy_wide_METAG)) %>% select(matches("cluster.*|Salinity_PSU"))  #get rid off problematic column with zero variance 
...
# formula_salinity <- reformulate(termlabels = clusters_model, response = 'Salinity_PSU')
#train - find parameters
# f <- paste("Salinity_PSU~", paste(clusters_model, collapse="+"))
# formula_salinity2 <-formula(f)
# formula_salinity <- formula("Salinity_PSU ~ ." )
#take part of the data as it gives error when trying to create long formula
#virusTrain <- na.omit(virusTrain)
...
# Transform data
geneabund_drep_marker_taxonomy_wide <- geneabund_drep_marker_taxonomy_long %>%
  ungroup() %>%
  select(-c("lineage", "level_5")) %>%
  pivot_wider(names_from = primary_cluster, values_from = counts, values_fill = 0, names_prefix = "cluster_") %>%
  distinct()

geneabund_drep_marker_taxonomy_wide_METAG <- geneabund_drep_marker_taxonomy_wide %>% 
  filter(data_type == "METAG")

# Split into train and test sets
set.seed(123)
trainIndex <- createDataPartition(geneabund_drep_marker_taxonomy_wide_METAG$Salinity_level, p = 0.8, list = FALSE, times = 1)

virusTrain <- geneabund_drep_marker_taxonomy_wide_METAG[trainIndex, ]
virusTest  <- geneabund_drep_marker_taxonomy_wide_METAG[-trainIndex, ]

# Ensure `Salinity_PSU` is numeric and check for NAs
virusTrain$Salinity_PSU <- as.numeric(virusTrain$Salinity_PSU)
#virusTrain <- na.omit(virusTrain)

# Ensure `Salinity_level` is a factor
virusTrain$Salinity_level <- as.factor(virusTrain$Salinity_level)

# Remove near-zero variance predictors
colzerovar <- nearZeroVar(virusTrain)
print(colzerovar)  # Debug: See what is removed

clusters_model <- colnames(virusTrain)[-c(colzerovar)]
clusters_model <- clusters_model[grepl("cluster", clusters_model)]
print(clusters_model)  # Debug: Check if valid predictors remain

# Ensure clusters_model is not empty
if (length(clusters_model) == 0) {
  stop("Error: No valid predictors after nearZeroVar filtering.")
}

# Cross-validation without manually defined folds
...
# Train the model
model_salinity <- train(
  x = data.matrix(virusTrain[clusters_model]), 
  y = virusTrain$Salinity_PSU, 
  method = "glmnet", 
  na.action = na.omit,  
...
```{r}
model_salinity$finalModel

predicted_y <- predict(model_salinity$finalModel, newx=as.matrix(geneabund_drep_marker_taxonomy_wide_METAG_test_x))
