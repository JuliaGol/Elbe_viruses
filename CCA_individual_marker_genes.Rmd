...
geneabund_drep_marker_taxonomy_wide <-  geneabund_drep_marker_taxonomy_long %>% pivot_wider(names_from=primary_cluster, values_from=counts, names_prefix="cluster_", values_fill=0) %>% distinct()
geneabund_drep_marker_taxonomy_wide_METAG <- geneabund_drep_marker_taxonomy_wide %>% filter(data_type == "METAG" & Sample_type !="Free_living")
...
for (i in 0:20) {
  ...
  ## Store the results
  temp <- data.frame(alpha=i/20, mse=mse, name=name, mae =mae)
  results <- rbind(results, temp)
}
```
```{r}

predicted_y <- predict(models[["alpha0.7"]], s=models[["alpha0.7"]]$lambda.1se, newx=as.matrix(geneabund_drep_marker_taxonomy_wide_METAG_test_x))
plot(y_test, predicted_y)
