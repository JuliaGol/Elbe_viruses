...
viruses_taxonomy_fractions <- geneabund_drep_marker_taxonomy_long %>% filter(counts!=0 ) %>% ggplot() + geom_bar(position ="fill", stat = "identity",  na.rm = TRUE, aes(x=AccessionNumber_TBDSven, y=counts, fill = level_5)) + scale_y_continuous(labels=scales::percent_format()) + facet_wrap(~data_type *Sample_type, scales = "free") + #facet_grid(rows=vars(data_type), cols=vars(Salinity_level), scales= "free") + 
...
#date as factor
geneabund_drep_marker_taxonomy_long_percent$Sample_date <- factor(geneabund_drep_marker_taxonomy_long_percent$Sample_date, levels= c("May-21", "Jul-21", "Feb-22", "Jun-22",  "May-22", "Nov-22"))
...
tiff("salinity_niche_taxa_bubble_plot.tiff", unit="px", width = 1000, height = 700)
xx
dev.off()
