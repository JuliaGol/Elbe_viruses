...
vir_genecluster_annot <- vir_genecluster_annot[, c("genome", "KO", "DESCRIPTION", "gene_cluster")] %>% merge(pathways[, c("Metabolism", "Pathway", "Present.AMG.KOs")], by.x="KO", by.y="Present.AMG.KOs") # find more pathways 
#add drep clusters 
#but first unify genome names
vir_genecluster_annot <- vir_genecluster_annot %>% 
  mutate(genome=str_remove(genome, "_[0-9]+$"))
vibrant_genecluster_drep = merge(drep_clusters, vir_genecluster_annot, by="genome", all.y = T) %>% select(genome, gene_cluster, KO, primary_cluster, secondary_cluster, DESCRIPTION, Metabolism, Pathway) %>% unique()
rm(vir_genecluster_annot)
...
```{r}
vibrant_genecluster_drep_abund<- readRDS(file="U:/IGB_phd/BICEST/virus/viral_abundances/vibrant_genecluster_drep_abund.RDS")
#visualisations
#fractions
...
tiff("metabolism_date_plot_test.tiff", unit="px", width = 1000, height = 1000)
...
tiff("metabolism_station_plot_test.tiff", unit="px", width = 1000, height = 1000)
...
vibrant_genecluster_drep_abund <- vibrant_genecluster_drep_abund %>% select("sampleid", "gene_cluster", "contig_id", "KO", "Metabolism", "Pathway", "counts", "Sample", "Stromkilometer", "Sample_type", "Sample_date", "Salinity_PSU",                "AccessionNumber_TBDSven", "data_type")
...
tiff("pathway_station_plot_test.tiff", unit="px", width = 1010, height = 1000)
