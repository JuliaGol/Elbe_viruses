...
matrix_res_stat_METAT_plot
ggplot(matrix_res_stat_METAT_plot, aes(x=Var1, y=Var2)) + 
    geom_tile(aes(fill = p_value)) + 
  scale_fill_gradient(low = "white", high = "steelblue") +
    geom_text(aes(label=round(R_statistic,3))) +
    geom_tile(data = matrix_res_stat_METAT_plot[which(matrix_res_stat_METAT_plot$p_value<0.05),], aes(x=Var1, y=Var2), color = "black", size = 2) + 
theme_minimal()
```
```{r}

  ggplot(data = matrix_res_stat_METAT_plot, aes(x=Var1, y=Var2)) +
  ...
  geom_text(vjust=0.76,hjust=0.49, aes(label = paste0(c("","")[(abs(sig) <= .05)+1], c("","")[(abs(sig) <= .001)+1], c("","*")[(abs(sig) <= .001)+1])))
```