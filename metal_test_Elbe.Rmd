...
viral_diversity_FL <- geneabund_drep_marker_taxa %>%
  select(contains("METAG")) %>% select(contains(sampleid_METAG$BioSample)) %>% select(contains(FL_accnum))#some METAG excluded from analysis - further selection by vector with metagenomes samples from otus tables (where samples were preselected) and by sampleid od the FL fraction 
...
metadata_METAT_FL <- metadata %>%
  filter(AccessionNumber_TBDSven %in% sampleid_METAT$BioSample & AccessionNumber_TBDSven %in% FL_accnum) 
j=0
z=0
...
#lets remove the confouded values
