...
matrix_res_stat_METAG <- matrix(nrow = list.dist.length, ncol = list.dist.length)
colnames(matrix_res_stat_METAG) <- c(names(list.METAG.dists))
matrix_res_stat_METAT <- matrix(nrow = list.dist.length, ncol = list.dist.length)
colnames(matrix_res_stat_METAT) <- c(names(list.METAT.dists))
matrix_res_sig_METAG <- matrix(nrow = list.dist.length, ncol = list.dist.length)
colnames(matrix_res_sig_METAG) <- c(names(list.METAG.dists))
matrix_res_sig_METAT <- matrix(nrow = list.dist.length, ncol = list.dist.length)
...
for (i in c(1:list.dist.length)) {
  ...
  if ((i <= 30 & j <= 30) | (i==j)){
    ...
    res <- mantel(dist.matrix, dist.matrix2, method = "spearman", permutations = 1, na.rm = TRUE)
    print(res)
    ...
    testvec <- cbind(testvec, res$signif)
  }
}
